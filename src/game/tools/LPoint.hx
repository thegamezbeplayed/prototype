...
class LPoint {
  ...
  public static inline function fromOther(other:LPoint){
    return new LPoint().setLevelCase(
      other.cx,
      other.cy,
      other.xr,
      other.yr);
  ...
  public inline function addAng(ang:Float,len:Float=1.){
    var xi = Math.cos(ang)*len;
    var yi = Math.sin(ang)*len;
    var tx = cx + xr + xi;
    var ty = cy + yr + yi;
    cx = M.floor(tx);
    cy = M.floor(ty);
  ...
  public inline function addVel(v:Velocity){
    var l = Math.sqrt(cxf*cxf+cyf*cyf);
    var a = Math.atan2(cyf,cxf);
    var tx = Math.cos(a)*(l+v.len);
    var ty = Math.sin(a)*(l+v.len);
    cx = M.floor(tx);
    cy = M.floor(ty);
    xr = M.decimals(tx);
    yr = M.decimals(ty);
  ...
  public static function getClusterPoints(amnt:Int, rad:Float, centerX:Float, centerY:Float){
    var plots: FixedArray<LPoint> = new FixedArray(amnt);

    for (i in 0...amnt) {
      var angle: Float = Lib.rnd(0, M.PI2); // Random angle
      var dist: Float = rad;

      var x: Float = centerX + Math.cos(angle) * dist;
      var y: Float = centerY + Math.sin(angle) * dist;
      var pt = fromPixels(x, y);
  ...
  public static function getSpiralPoints(amnt:Int,rad:Float,centerX:Float,centerY:Float){

    var plots : FixedArray<LPoint> = new FixedArray(amnt);

    var a:Int = 0; // Starting radius
    var b:Float = rad; // Growth rate
    var thetaMax:Float = 10 * Math.PI; // Maximum angle (10 rotations)
    var points:Int = amnt;

    for (i in 0...points) {
      var t:Float = (i / points) * thetaMax; // Calculate theta
      var r:Float = a + b * t; // Calculate radius
      var x:Float = centerX + r * Math.cos(t); // Convert to Cartesian X
      var y:Float = centerY + r * Math.sin(t); // Convert to Cartesian Y
      var pt = fromPixels(x,y);
      plots.push(pt);
    }
    return plots;
  ...
  public static function getEllipsePoints(amnt:Int, radiusX:Float, radiusY:Float, centerX:Float, centerY:Float) {
    var plots: FixedArray<LPoint> = new FixedArray(amnt);

    var a:Float = 0; // Starting radius
    var bX:Float = radiusX;// / (10 * Math.PI); // Growth rate for X-axis
    var bY:Float = radiusY;// / (10 * Math.PI); // Growth rate for Y-axis
    var thetaMax:Float = 10 * Math.PI; // Maximum angle (10 rotations)

    for (i in 0...amnt) {
        var t:Float = (i / amnt) * thetaMax; // Angle for the current point
        var rX:Float = a + bX * t; // Growing radius for X-axis
        var rY:Float = a + bY * t; // Growing radius for Y-axis
        var x:Float = centerX + rX * Math.cos(t); // Elliptical X component
      var y:Float = centerY + rY * Math.sin(t); // Elliptical Y component
      var pt = fromPixels(x, y);
      plots.push(pt);
    }
  ...
  public static function getFlyingVPoints(amnt:Int, spread:Float, centerX:Float, centerY:Float, angle:Float = 0):Array<LPoint> {
    var plots:Array<LPoint> = [];
    var half = Math.floor(amnt / 2);

    for (i in 0...amnt) {
      var offset = i - half;
      var x = centerX + offset * spread * Math.cos(angle) - Math.abs(offset) * spread * 0.5;
      var y = centerY + offset * spread * Math.sin(angle) + Math.abs(offset) * spread * 0.5;

      plots.push(fromCase(x, y));
    }
    return plots;
  }

  public static function getFurthestPt(points:Array<LPoint>, loc:LPoint){
    var dh = new dn.DecisionHelper(points);
    dh.score( p->p.distCase(loc) );
    var pt = dh.getBest();

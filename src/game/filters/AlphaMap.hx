...
class AlphaMap extends h2d.filter.Shader<InternalShader> {
...
private class InternalShader extends h3d.shader.ScreenShader {
  static var SRC = {
	...
	var rep = gradientMap.get( vec2(pixel.a, paletteY) );
	switch (mode){
	  case 0: // Full gradient map
	  pixelColor = vec4( mix(pixel.rgb, rep.rgb, intensity ), pixel.a);
	  case 1: // Only lights
	  pixelColor = vec4( mix(rep.rgb, rep.rgb, intensity*lum ), pixel.a);
	  case 2: // Only shadows
